...
<header class="fixed inset-x-0 top-0 z-50 border-b border-sand/10 bg-slate/70 backdrop-blur-md">
	<div class="flex w-full items-center justify-between px-6 py-4 md:px-12 lg:px-20">
		<a
			...
			class="font-display text-lg font-bold tracking-wide text-amber"
		>
			GeauxHack '26
		</a>
		...
		<div class="flex items-center gap-4">
			...
			>
				...
				></span>
				<span class="h-0.5 w-6 rounded-full bg-sand transition-opacity" class:opacity-0={mobileOpen}
				></span>
				<span
					class="h-0.5 w-6 rounded-full bg-sand transition-transform"
					class:-rotate-45={mobileOpen}
		...
		<nav class="flex flex-col gap-1 border-t border-sand/10 bg-slate/95 px-6 py-4 md:hidden">
			{#each links as link (link.href)}
				<a
					href={link.href}
					onclick={(e) => goTo(e, link.href)}
					class="rounded-md px-3 py-3 font-mono-retro text-xs tracking-[0.2em] text-sand/90 uppercase transition hover:bg-sand/10 hover:text-amber"
				>
					{link.label}
				</a>
			{/each}
			<a
				...
				target="_blank"
				rel="noreferrer"
				class="mt-2 rounded-full bg-crimson px-5 py-3 text-center font-display text-sm font-semibold text-sand"
			>
				Apply
			</a>
		</nav>
